...
class ServiceTypes{
    ...
    */
    ...
    public function applyPriceLogic($requestarr,$iflag=0){
        ...
        $City_id=$requestarr['city_id'];
        

        $service_type = ServiceType::with(['fareestimatemapping'=> function ($skill) use ($City_id){
                   
                        
                         $skill->where('city_id',$City_id);
                    }])
                ->whereId($requestarr['service_type'])
                ->first();
        ...
        $per_minute=$service_type->fareestimatemapping[0]->minute; //PM
        $per_hour=$service_type->fareestimatemapping[0]->hour; //PH
        $per_kilometer=$service_type->fareestimatemapping[0]->price; //PKM
        $base_distance=$service_type->fareestimatemapping[0]->distance; //BD       
        $base_price=$service_type->fareestimatemapping[0]->fixed; //BP

        if($service_type->fareestimatemapping[0]->calculator == 'MIN') {
            //BP+(TM*PM)
            $price = $base_price+($total_minutes * $per_minute);
        } else if($service_type->fareestimatemapping[0]->calculator == 'HOUR') {
            //BP+(TH*PH)
            $price = $base_price+($total_hours * $per_hour);
        } else if($service_type->fareestimatemapping[0]->calculator == 'DISTANCE') {
            //BP+((TKM-BD)*PKM)  
        ...
        } else if($service_type->fareestimatemapping[0]->calculator == 'DISTANCEMIN') {
            //BP+((TKM-BD)*PKM)+(TM*PM)
            if($base_distance>$total_kilometer){
                $price = $base_price+($total_minutes * $per_minute);
            }
        ...
        } else if($service_type->fareestimatemapping[0]->calculator == 'DISTANCEHOUR') {
            //BP+((TKM-BD)*PKM)+(TH*PH)
            if($base_distance>$total_kilometer){
                $price = $base_price+($total_hours * $per_hour);
            }
        ...
        $fn_response['calculator']=$service_type->fareestimatemapping[0]->calculator;
