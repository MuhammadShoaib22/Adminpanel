...
class Helper
{
    ...
    public function formatPagination($pageobj,$query_string = array()){
        $results = new \stdClass();
        if(!empty($query_string)){
            $results->links=$pageobj->appends($query_string)->links();
        }else{
    ...
    public static function generate_request_id($type) {
        ...
        else if($type=='provider_wallet'){
            $tr_str='PWALLET';
            $typecount=ProviderWallet::where('transaction_alias','LIKE','%PWALLET%')->count();
        }
        else{
            $tr_str='FSET';
        }
