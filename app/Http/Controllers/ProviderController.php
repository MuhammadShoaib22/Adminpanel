...
class ProviderController extends Controller
{
    ...
    {
        $subscription = SubscriptionHistories::whereprovider_id(Auth::user()->id)->wherestatus('Active')->first();

        return view('provider.index',compact('subscription')); 
    }
    ...
    public function subscribe_cards(Request $request)
    {
        try{

            $cards = Card::where('provider_id',Auth::user()->id)->orderBy('created_at','desc')->get();
            if($request->ajax())
            {
                return $cards; 
            } 
            return view('provider.payment.cards',compact('cards')); 

        } catch(Exception $e){
            return response()->json(['error' => $e->getMessage()], 500);
        }
    } 
    public function subscribe_card_store(Request $request)
    {
        ...
        try{
                ...
                $exist = Card::where('provider_id',Auth::user()->id)
                ...
                if($exist == 0){
                    ...
                    $create_card->provider_id = Auth::user()->id;
    ...
    public function subscribe_card_destroy(Request $request)
    {

        $this->validate($request,[
                'card_id' => 'required|exists:cards,card_id,provider_id,'.Auth::user()->id,
            ]);
    ...
    {
        if(Auth::user()->subscribe_stripe_cust_id != null){

            return Auth::user()->subscribe_stripe_cust_id;
            ...
            try{
                ...
                Provider::where('id',Auth::user()->id)->update(['subscribe_stripe_cust_id' => $customer['id']]);
                return $customer['id'];
