...
class UserApiController extends Controller
{
    ...
    public function details(Request $request){
        ...
        try{
            ...
            if($user = User::find(Auth::user()->id)){
                ...
                $user->city_name = Cities::findOrfail($user->city_id)->name; 
                return $user;
    ...
    public function send_request(Request $request) {
        ...
        $this->validate($request, [
                ...
                'use_wallet' => 'numeric',
                'payment_mode' => 'required|in:CASH,CARD,PAYPAL,EASYPAISA',
        ...
        if(!empty($request->present_country_code))
        {
                $county= Country::wherecode($request->present_country_code)->first(); 
               
                $user=User::find(Auth::user()->id);
                $user->present_riding_country_id=$county->id;
                $user->save();
                $currency=$county->currency_symbol;
                $currency_code=$county->currency_code;
        }
        ...
        try{
            ...
            $UserRequest->city_id = $request->city_id;
            $UserRequest->currency = $currency;
            $UserRequest->currency_code=$currency_code;
            $UserRequest->country_code =$request->present_country_code;
        ...
        try{
            ...
            if(in_array($UserRequest->status, ['SEARCHING','STARTED','ARRIVED','SCHEDULED'])) {
                ...
                { 
                    ...
                    $user->wallet_balance -=Setting::get('cancellation_fee');
                    $user->save();
                }
    ...
    public function respective_city($id)
    {
        ...
        $city = Cities::where('country_code', $country->code)->where('status',1)->get();
    ...
    public function cityvalidate(Request $request)
    {

         $cityname=$request->city;
        $statename=$request->state;
        $statenamelong=$request->statelong;
        ...
        $cityExist = Cities::where('country_code', $countryCode)->where('status',1)->where('name','like','%'.$cityname.'%')->first();

        if(count($cityExist) ==0) 
        {

            $cityname =$request->city.', '.$request->state;
            ...
            if(count($cityExist) ==0)
            {
                ...
                if(count($cityExist) ==0)
                {
                        ...
                        $cityname =$request->city.' '.$request->statelong;                        
                        $cityExist = Cities::where('country_code', $countryCode)->where('status',1)->where('name','like','%'.$cityname.'%')->first();
                        if(count($cityExist) ==0)
                        {
        ...
        if(count($cityExist)>0)
        {
               ...
               return response()->json(['message'=>'success','city_id'=> $cityExist->id,'country_code'=>$countryCode]);
            
        }
        else
        {
            
             return response()->json(['message'=>'Service is not available for this city']);
        }
    ...
    public function languagechange(Request $request)
    {
        ...
        $this->app->setLocale(\Cookie::get('lang', $language));
    ...
    public function countrycodedropdown($countrycode)
    { 
    ...
    public function flow_payment_change(Request $request){
