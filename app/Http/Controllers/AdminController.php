...
class AdminController extends Controller
{
    ...
    public function dashboard(Request $request)
    {
        try{
            ...
            $rides = UserRequests::has('user')->where('country_code',$request->country_code)->orderBy('id','desc')->get();
            $cancel_rides = UserRequests::where('status','CANCELLED')->where('country_code',$request->country_code);
            $scheduled_rides = UserRequests::where('status','SCHEDULED')->where('country_code',$request->country_code)->count();
            $user_cancelled = UserRequests::where('status','CANCELLED')->where('cancelled_by','USER')->where('country_code',$request->country_code)->count();
            $provider_cancelled = UserRequests::where('status','CANCELLED')->where('country_code',$request->country_code)->where('cancelled_by','PROVIDER')->count();
            $cancel_rides = $cancel_rides->count();
            $service = ServiceType::count();
            $fleet = Fleet::where('country_id',@$country_code->id)->count();
            $provider = Provider::where('country_id',@$country_code->id)->count();
            
             // $revenue = UserRequestPayment::select('user_requests.currency_code',DB::raw('SUM(user_request_payments.total) AS total'),DB::raw('count(*) as count'))
             // ->leftjoin('user_requests', 'user_request_payments.request_id', '=', 'user_requests.id')
             // ->leftjoin('countries', 'countries.code', '=', 'user_requests.country_code')             
             // ->groupBy('user_requests.currency_code')->get();
             $revenue = UserRequestPayment::select('user_requests.currency_code',DB::raw('SUM(user_request_payments.total) AS total'),DB::raw('count(*) as count'))
             ->leftjoin('user_requests', 'user_request_payments.request_id', '=', 'user_requests.id')
             ->where('user_requests.country_code',$request->country_code)             
             ->groupBy('user_requests.currency_code')->get();
        
            $wallet['tips'] = UserRequestPayment::leftjoin('user_requests', 'user_request_payments.request_id', '=', 'user_requests.id')->where('user_requests.country_code',$request->country_code)->sum('tips');
            $providers = Provider::where('country_id',@$country_code->id)->take(10)->orderBy('rating','desc')->get();
            $wallet['admin'] = AdminWallet::where('requested_country_id',@$country_code->id)->sum('amount');
            $wallet['provider_debit'] = Provider::select(DB::raw('SUM(CASE WHEN wallet_balance<0 THEN wallet_balance ELSE 0 END) as total_debit'))->where('country_id',@$country_code->id)->get()->toArray();
            $wallet['provider_credit'] = Provider::select(DB::raw('SUM(CASE WHEN wallet_balance>=0 THEN wallet_balance ELSE 0 END) as total_credit'))->where('country_id',@$country_code->id)->get()->toArray();
            $wallet['fleet_debit'] = Fleet::select(DB::raw('SUM(CASE WHEN wallet_balance<0 THEN wallet_balance ELSE 0 END) as total_debit'))->where('country_id',@$country_code->id)->get()->toArray();
            $wallet['fleet_credit'] = Fleet::select(DB::raw('SUM(CASE WHEN wallet_balance>=0 THEN wallet_balance ELSE 0 END) as total_credit'))->where('country_id',@$country_code->id)->get()->toArray();

            $wallet['admin_tax'] = AdminWallet::where('requested_country_id',@$country_code->id)->where('transaction_type',9)->sum('amount');
            $wallet['admin_commission'] = AdminWallet::where('requested_country_id',@$country_code->id)->where('transaction_type',1)->sum('amount');
            $wallet['admin_discount'] = AdminWallet::where('requested_country_id',@$country_code->id)->where('transaction_type',10)->sum('amount');
    ...
    public function settings_payment_store(Request $request)
    {
        ...
        Setting::set('commission_percentage_dispatch', $request->commission_percentage_dispatch);
        ...
        Setting::set('default_waiting_minute', $request->default_waiting_minute);
        Setting::set('waiting_minute_price', $request->waiting_minute_price);
        Setting::set('cancellation_fee', $request->cancellation_fee);
        Setting::set('incentive_rides', $request->incentive_rides);
        Setting::set('incentive_amount', $request->incentive_amount);
        Setting::save();
    ...
    {
        try {
             $payments = UserRequests::where('country_code',$request->country_code)->where('paid', 1)
                    ->has('user')
                    ->has('provider')
                    ->has('payment')
            ...
            if($request->has('country_code')){
             ...
             $pagination=(new Helper)->formatPagination($payments,$data);       
    ...
    public function statement(Request $request,$type = ''){
        ...
        try{
            ...
            $rides = UserRequests::with('payment')->where('country_code',$request->country_code)->orderBy('id','desc');

            $cancel_rides = UserRequests::where('country_code',$request->country_code)->where('status','CANCELLED');
            $revenue = UserRequestPayment::select(\DB::raw(
                           'SUM(ROUND(user_request_payments.fixed) + ROUND(user_request_payments.distance)) as overall, SUM(ROUND(user_request_payments.commision)) as commission'
                       ))->leftjoin('user_requests', 'user_request_payments.request_id', '=', 'user_requests.id')->where('user_requests.country_code',$request->country_code);

            if($type == 'today'){

                $rides->where('created_at', '>=', Carbon::today());
                $cancel_rides->where('created_at', '>=', Carbon::today());
                $revenue->where('user_request_payments.created_at', '>=', Carbon::today());

            }elseif($type == 'monthly'){

                $rides->where('created_at', '>=', Carbon::now()->month);
                $cancel_rides->where('created_at', '>=', Carbon::now()->month);
                $revenue->where('user_request_payments.created_at', '>=', Carbon::now()->month);

            }elseif($type == 'yearly'){

                $rides->where('created_at', '>=', Carbon::now()->year);
                $cancel_rides->where('created_at', '>=', Carbon::now()->year);
                $revenue->where('user_request_payments.created_at', '>=', Carbon::now()->year);
                ...
                if($request->from_date == $request->to_date) {
                    ...
                    $revenue->whereDate('user_request_payments.created_at', date('Y-m-d', strtotime($request->from_date)));
                } else {
                    ...
                    $revenue->whereBetween('user_request_payments.created_at',[Carbon::createFromFormat('Y-m-d', $request->from_date),Carbon::createFromFormat('Y-m-d', $request->to_date)]);
                }
            }
    ...
    public function statement_today(Request $request){
        return $this->statement($request,'today');
    }
     ...
     * @return \Illuminate\Http\Response
     */
    public function statement_monthly(Request $request){
        return $this->statement($request,'monthly');
    }
     ...
     * @return \Illuminate\Http\Response
     */
    public function statement_yearly(Request $request){
        return $this->statement($request,'yearly');
    }
    ...
    public function statement_range(Request $request){
        return $this->statement($request,'range');
    }
    ...
    public function statement_provider(Request $request){
        ...
        try{
            ...
            $Providers = Provider::where('country_id',@$country_code->id)->paginate($this->perpage);

            $pagination=(new Helper)->formatPagination($Providers);

            foreach($Providers as $index => $Provider){

                $Rides = UserRequests::where('country_code',$request->country_code)->where('provider_id',$Provider->id)
                ...
                $Providers[$index]->payment = UserRequestPayment::whereIn('request_id', $Rides)
                                ...
                                ))->leftjoin('user_requests', 'user_request_payments.request_id', '=', 'user_requests.id')->where('user_requests.country_code',$request->country_code)->get();
            }
    ...
    public function transactions(Request $request){
        ...
        try{
            ...
            $wallet_transation = AdminWallet::where('requested_country_id',@$country_code->id)->orderBy('id','desc')->paginate(Setting::get('per_page', '10'));
    ...
    {   
        ...
        try{
            ...
            $insert->provider_id = $request->provider_id; 
            ...
            if($request->ajax()){
                ...
                return back()->with('flash_success',trans('api.push.subscribed'));
            }
