		...
		<div class="col-lg-3 col-md-6 col-xs-12">
			<div class="box box-block bg-white tile tile-1 mb-2">
				...
				<div class="t-content">
					...
					<h1 class="mb-1">{{currency(@$revenue[0]->total,@Session::get('country_code','IN'))}}</h1>
					...
					<table class="table mb-md-0">
						<tbody>
							...
							<tr>
								...
								<td class="text-success">{{currency($wallet['admin'],@Session::get('country_code','IN'))}}</td>
							</tr>
							<tr>
								<th scope="row">Provider Credit</th>
								@if($wallet['provider_credit'])
								@php($total=$total-$wallet['provider_credit'][0]['total_credit'])
									<td class="text-success">{{currency($wallet['provider_credit'][0]['total_credit'],@Session::get('country_code','IN'))}}</td>
								@else
							...
							<tr>
								...
								@if($wallet['provider_debit'])
									...
									<td class="text-danger">{{currency($wallet['provider_debit'][0]['total_debit'],@Session::get('country_code','IN'))}}</td>
								@else
							...
							<tr>
								...
								@if($wallet['fleet_credit'])
									...
									<td class="text-success">{{currency($wallet['fleet_credit'][0]['total_credit'],@Session::get('country_code','IN'))}}</td>
								@else
									<td class="text-success">{{currency()}}</td>		
								@endif	
							</tr>
							<tr>
								...
								@if($wallet['fleet_debit'])								
									<td class="text-danger">{{currency($wallet['fleet_debit'][0]['total_debit'],@Session::get('country_code','IN'))}}</td>
								@else
									<td class="text-danger">{{currency()}}</td>		
								@endif	
							</tr>
							<tr>
								...
								<td class="text-success">{{currency($wallet['admin_commission'],@Session::get('country_code','IN'))}}</td>
							</tr>
							<tr>
								<th scope="row">Discount</th>
								<td class="text-danger">{{currency($wallet['admin_discount'],@Session::get('country_code','IN'))}}</td>
							</tr>
							<tr>
								@php($total=$total-($wallet['admin_tax']))
								<th scope="row">Tax Amount</th>
								<td class="text-success">{{currency($wallet['admin_tax'],@Session::get('country_code','IN'))}}</td>
							</tr>

							<tr>
								<th scope="row">Tips</th>
								<td class="text-danger">{{currency($wallet['tips'],@Session::get('country_code','IN'))}}</td>
							</tr>
							<!-- <tr>
