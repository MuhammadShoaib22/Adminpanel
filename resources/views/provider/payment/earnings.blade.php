...
<div class="pro-dashboard-head">
        <div class="container">
            ...
            <a href="{{url('provider/subscription')}}"" class="pro-head-link">@lang('admin.subscription_history.subscription')</a> 
                                    ...
                                    <table class="table table-condensed table-responsive" style="border-collapse:collapse;">
                                        ...
                                        @foreach($weekly as $day)
                                            <tr>
                                                ...
                                                <td class="text-right">
                                                @if($day->payment != "")
                                                {{currency($day->payment->provider_pay,@App\Country::find(Auth::user()->country_id)->code)}}
                                                @else
                                                {{currency(0.00,@App\Country::find(Auth::user()->country_id)->code)}}
                                                @endif
                                                </td>
                                            </tr>
                                        @endforeach
                            ...
                            <div class="earning-acc pad20 border-top">
                                <div class="row no-margin">
                                    ...
                                    <div class="pull-right trip-right">
                                        <p class="earning-cost no-margin">{{currency($weekly_sum,@App\Country::find(Auth::user()->country_id)->code)}}</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                            ...
                            <table class="earning-table table table-responsive">
                                ...
                                @foreach($fully as $each)
                                    <tr>
                                        ...
                                        <td> 
                                            @if($each->payment != "")
                                            {{currency($each->payment->total,@App\Country::find(Auth::user()->country_id)->code)}}
                                            @else
                                            {{currency(0.00,@App\Country::find(Auth::user()->country_id)->code)}}
                                            @endif
                                        </td>
                                        <td>
                                        	@if($each->payment != "")
                                        		...
                                        		$fully_sum += $each_sum;
                                        		?>
                                        		{{currency($each_sum,@App\Country::find(Auth::user()->country_id)->code)}}
                                            @else
                                            -    
                                        	@endif
                                        </td>
                                        <td>@if($each->status=='CANCELLED')- @else{{currency($fully_sum,@App\Country::find(Auth::user()->country_id)->code)}}@endif</td>
                                    </tr>
                                @endforeach
...
<script type="text/javascript">
	document.getElementById('set_fully_sum').textContent = "{{currency($fully_sum,@App\Country::find(Auth::user()->country_id)->code)}}";
</script>
@endsection