    ...
    <script type="text/jsx">
		...
		var MainComponent = React.createClass({
			...
			getInitialState: function () {
               return {data: [], currency : "{{$request->currency_code}}"};
            },
		...
		var SwitchState = React.createClass({
			...
			render: function(){
				...
				if(this.props.checkState != ""){
					...
					}else if((this.props.checkState.status == 'DROPPED' || this.props.checkState.status == 'COMPLETED') && this.props.checkState.payment_mode == 'EASYPAISA' && this.props.checkState.paid == 0){
						return (
							<div>
								<DroppedAndPaisa checkState={this.props.checkState} currency={this.props.currency} />
							</div>
						);
					}else if(this.props.checkState.status == 'COMPLETED'){
						return (
		...
		var DroppedAndCash = React.createClass({
			...
			render: function(){
				let fixval=2;
				let waiting='{{Setting::get('waiting_minute_price')}}';
				return (
				<div>
					<div className="status">
		                <h6>@lang('user.status')</h6>
		            ...
		            <br/>
		            	...
		            	<dl className="dl-horizontal left-right">
                            ...
                            <dd>{this.props.currency} {this.props.checkState.payment.distance.toFixed(fixval)}</dd>
                            @endif
                            {this.props.checkState.payment.waiting_charge ?
	                            <span>
	                            <dt>@lang('user.ride.waiting_charge') ({this.props.checkState.payment.waiting_charge/waiting} Mins)</dt>
	                            <dd>{this.props.currency}{this.props.checkState.payment.waiting_charge.toFixed(fixval)}</dd>
	                        	</span> 
                             : ''
                            }
                            {this.props.checkState.payment.wallet_deduction ?
	                            <span>
	                            <dt>@lang('user.ride.wallet_deduction') </dt>
	                            <dd>{this.props.currency}{this.props.checkState.payment.wallet_deduction.toFixed(fixval)}</dd>
	                        	</span> 
                             : ''
                            }
                            ...
                            {this.props.checkState.payment.discount ?
                            	...
                            	<dd>{this.props.currency}{<this class="props"></this>.checkState.payment.discount.toFixed(fixval)}</dd>  
                            	</span>
                            : ''
                            }
		...
		var DroppedAndCard = React.createClass({
			...
			var DroppedAndPaisa = React.createClass({
			...
			render: function(){
				...
				return (
				<div>
					<form method="POST" action="{{url('/payment/paisa')}}">
						{{ csrf_field() }}</input>         d		            	<h5><strong>@lang('user.ride.ride_details')</strong></h5>
		            	<dl className="dl-horizontal left-right">
		            		<dt>@lang('user.booking_id')</dt>
				            <dd>{this.props.checkState.booking_id}</dd>
				            <dt>@lang('user.service_type')</dt>
			                ...
			                <dd>
			                	<div className="rating-outer">
		                            <input type="hiddsen" value={this.props.checkState.provider.rating} name="rating" className="rating" disabled/>
		                        </div>
			                </dd>
		            		<dt>@lang('user.payment_mode')</dt>
                        	<dd>{this.props.checkState.payment_mode}</dd>
		            	...
		            	<dl className="dl-horizontal left-right">
                            ...
                            <dd>{this.props.currency} {this.state.total.toFixed(fixval)}</dd> 
                            <dt className="big">@lang('user.ride.amount_paid')</dt>
                            <dd className="big">{this.props.currency} {this.state.total.toFixed(fixval)}</dd>
                            <dt>@lang('user.ride.tips')</dt>
                            <dd>{this.props.currency}<input type="number" min="0" name="tips" id="tips" onChange={this.handletipsChange}/></dd>
                        </dl>
                    	<button type="submit" className="full-primary-btn fare-btn">CONTINUE TO PAY - {this.props.currency} {this.state.total.toFixed(fixval)}</button>   
                    </form>
		        </div>
				);
			}
