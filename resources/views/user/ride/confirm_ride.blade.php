            ...
            <div class="col-md-6">
                ...
                {{ csrf_field() }}
                    <dl class="dl-horizontal left-right">
                        ...
                        <dd>{{symbolcurrency($fare->estimated_fare,$present_riding_currency_code)}}</dd>
                        <dt>@lang('user.promocode')</dt>
                        <dd id="promo_amount">{{symbolcurrency('',$present_riding_currency_code)}}</dd>
                        <hr>
                        <dt>@lang('user.total')</dt>
                        <dd id="total_amount">{{symbolcurrency($fare->estimated_fare - 0,$present_riding_currency_code)}}</dd>
                        <hr>
                        @if($present_riding_country_id==1)
                          ...
                          <br>
                              ...
                              <dd>{{symbolcurrency(Auth::user()->wallet_balance,$present_riding_currency_code)}}</dd>
                          @endif
                          @endif
                    </dl>
                    ...
                    <input type="hidden" name="service_type" value="{{Request::get('service_type')}}">
                     ...
                     <input type="hidden" name="country_code" value="{{Request::get('city_id')}}">


                    <input type="hidden" name="present_country_code" value={{$present_country_code}}>
                    ...
                    @endif
                    @if(Setting::get("EASYPAISA")==1 || Setting::get("CASH")==1)
    ...
    <script type="text/javascript">
        ...
        $('#promocode').on('change', function() {
          ...
          var percent_total = estimate * percentage/100;
             ...
             $("#promo_amount").html("{{$present_riding_currency_code}}"+promo.toFixed(2));
             $("#total_amount").html("{{$present_riding_currency_code}}"+(estimate-promo).toFixed(2));
          });
    </script>
@endsection